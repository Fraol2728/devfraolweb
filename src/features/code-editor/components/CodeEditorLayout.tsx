    ...
    <div>
      <div className={`ce-tree-row ${activeFileId === node.id ? "active" : ""}`} style={{ paddingLeft: `${depth * 12 + 8}px` }}>
        ...
        <div className="ce-tree-actions">
          {isFolder && (
            <button onClick={() => {
              const name = window.prompt("File name", "new-file.ts");
...
export const CodeEditorLayout = () => {
  ...
  return (
    <section className="ce-root">
      <header className="ce-menu">{menus.map((m) => <button key={m}>{m}</button>)}</header>
      ...
      <div className="ce-main" style={{ gridTemplateColumns: `${leftOpen ? `${leftWidth}px` : "0px"} minmax(0,1fr) ${rightOpen ? `${rightWidth}px` : "0px"}` }}>
          ...
          {leftOpen && (
            <>
              <div className="ce-pane-header">
                Explorer
                <button onClick={() => useFilesStore.getState().createFile(null, `file-${Date.now()}.ts`)}>New File</button>
              </div>
              <div className="ce-tree">{tree.map((node) => <TreeNode key={node.id} node={node} depth={0} />)}</div>
              <div className="ce-resizer-x" onPointerDown={(e) => {
                const start = e.clientX;
                const width = leftWidth;
                const move = (ev: PointerEvent) => setLeftWidth(Math.max(180, Math.min(420, width + ev.clientX - start)));
                const up = () => {
        ...
        <main className="ce-center">
          <div className="ce-tabs">
            {tabs.map((tab) => (
              <div key={tab.id} className={`ce-tab ${tab.id === activeFileId ? "active" : ""}`}>
          ...
          <div className="ce-editor-wrap">
            {activeFile ? (
              <Suspense fallback={<div className="ce-loading">Loading Monaco...</div>}>
                <MonacoEditor file={activeFile} onChange={handleChange} />
              </Suspense>
            ) : (
              <div className="ce-loading">No file selected.</div>
            )}
          </div>
        </main>
        ...
        <aside className="ce-right">
          {rightOpen && (
            <>
              <div className="ce-pane-header">Preview</div>
              <iframe
                className="ce-preview"
                title="preview"
                srcDoc={`<html><body style='background:#111;color:#eee;font-family:sans-serif'>${activeFile?.content ?? ""}</body></html>`}
              />
              <div className="ce-resizer-x left" onPointerDown={(e) => {
                const start = e.clientX;
                const width = rightWidth;
                const move = (ev: PointerEvent) => setRightWidth(Math.max(240, Math.min(620, width - (ev.clientX - start))));
                const up = () => {
                  window.removeEventListener("pointermove", move);
                  window.removeEventListener("pointerup", up);
                };
      ...
      <section className="ce-terminal" style={{ height: terminalOpen ? `${terminalHeight}px` : "0px" }}>
        {terminalOpen && (
          <>
            <div className="ce-resizer-y" onPointerDown={(e) => {
              const start = e.clientY;
              const height = terminalHeight;
              const move = (ev: PointerEvent) => setTerminalHeight(Math.max(120, Math.min(360, height - (ev.clientY - start))));
              const up = () => {
                window.removeEventListener("pointermove", move);
                window.removeEventListener("pointerup", up);
              };
            ...
            <div className="ce-pane-header">
              ...
              <div>
                ...
                <button onClick={toggleTerminal}>Hide</button>
                <button onClick={toggleRight}>Toggle Preview</button>
              </div>
            </div>
            <div className="ce-logs">{logs.map((log) => <div key={log.id}>{log.text}</div>)}</div>
          </>
        )}
      </section>

      {commandPaletteOpen && (
        <div className="ce-modal-backdrop" onClick={closeCommandPalette}>
