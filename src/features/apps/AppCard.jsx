...
export const AppCard = ({ app, index, onSelectTool, isFavorited, onToggleFavorite }) => {
  const Icon = app.icon;
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between">
        ...
        <button type="button" onClick={() => onToggleFavorite?.(app.id)} className={cn("rounded-lg border p-2 transition-colors", isFavorited ? "border-[#FF3B30]/50 bg-[#FF3B30]/10 text-[#FF3B30]" : "border-border text-foreground/70 hover:border-[#FF3B30]/50 hover:text-[#FF3B30]")} aria-label={`Toggle favorite for ${app.name}`}>
