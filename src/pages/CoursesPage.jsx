...
export const CoursesPage = () => {
  ...
  useSeoMeta({
    ...
    description: "Explore programming, graphics design, and computer operation courses with live backend data.",
  });
  ...
  const categories = useMemo(() => {
    const discovered = Array.from(new Set(courses.map((course) => course.category).filter(Boolean)));
    return [...CATEGORY_ORDER, ...discovered.filter((item) => !CATEGORY_ORDER.includes(item))];
  }, [courses]);
  ...
  return (
    <main className="mx-auto w-full max-w-[1320px] space-y-7 px-4 pb-16 sm:px-6 lg:px-8">
      <section className="rounded-3xl border border-white/10 bg-gradient-to-r from-[#130f1b] via-[#1a1727] to-[#1e1c2e] p-6 sm:p-9">
        ...
        <p className="mt-3 max-w-2xl text-sm text-white/75 sm:text-base">Live course data connected to the admin panel with category filtering and smart search.</p>
      </section>
      ...
      {loading ? (
        <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <CourseSkeleton key={`course-skeleton-${index}`} />
          ))}
        </section>
      ) : null}
        ...
        <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {visibleCourses.map((course, index) => (
            <motion.article
              ...
              className="overflow-hidden rounded-2xl border border-white/10 bg-[#0d0d13]/90"
            >
              <Link to={`/course/${course.slug}`} className="block">
                ...
                <div className="space-y-3 p-4">
                  ...
                  <p className="text-xs text-white/60">Instructor: Dev Fraol</p>
                  <span className="inline-flex w-full justify-center rounded-xl bg-gradient-to-r from-[#ff544a] to-[#ff3b30] px-4 py-2 text-sm font-semibold text-white">
                    View course
                  </span>
                </div>
              </Link>
